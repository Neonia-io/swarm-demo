...
use rmcp::ServiceExt;
use rmcp::service::{RunningService, Peer};
use rmcp::RoleClient;
...
use crate::utils::{queue_pop, find_valid_position};
use crate::fetch_resource;

pub fn spawn_producer_task(app_producer: AppHandle, worker_mcp_client: Arc<RunningService<RoleClient, InitializeRequestParams>>, q_name_clone: String)
{
    tokio::spawn(async move {
        println!("{}", "[PRODUCER] Initializing with Static Assets...".cyan());
        
        let predef_components = [
            PredefComponent { name: "IRON MINE", width: 140.0, height: 140.0, svg_file: "iron_mine.svg" },
            PredefComponent { name: "COPPER MINE", width: 140.0, height: 140.0, svg_file: "copper_mine.svg" },
            PredefComponent { name: "SMELTER", width: 180.0, height: 180.0, svg_file: "smelter.svg" },
            PredefComponent { name: "ASSEMBLER", width: 200.0, height: 200.0, svg_file: "assembler.svg" },
            PredefComponent { name: "WAREHOUSE", width: 240.0, height: 160.0, svg_file: "warehouse.svg" },
            PredefComponent { name: "POWER PLANT", width: 220.0, height: 220.0, svg_file: "power_plant.svg" },
            PredefComponent { name: "TRANSPORT HUB", width: 120.0, height: 120.0, svg_file: "transport_hub.svg" },
            PredefComponent { name: "FLUID PUMP", width: 100.0, height: 160.0, svg_file: "fluid_pump.svg" },
            PredefComponent { name: "RESEARCH LAB", width: 260.0, height: 260.0, svg_file: "research_lab.svg" },
            PredefComponent { name: "SOLAR ARRAY", width: 160.0, height: 240.0, svg_file: "solar_array.svg" },
        ];

        let mut batch_count = 0;
        loop {
            let _ = app_producer.emit("agent-event", AgentEvent { agent_id: "Producer".into(), status: "working".into(), details: "Generating Mechanism Segment...".into(), target_x: None, target_y: None });

            let mut summary = String::new();
            let mut sector_components = vec![predef_components[9].clone()];
            if batch_count % 3 == 0 {
                sector_components.extend([predef_components[0].clone(), predef_components[2].clone(), predef_components[3].clone(), predef_components[4].clone()]);
            } else if batch_count % 3 == 1 {
                sector_components.extend([predef_components[1].clone(), predef_components[2].clone(), predef_components[3].clone(), predef_components[6].clone()]);
            } else {
                sector_components.extend([predef_components[7].clone(), predef_components[5].clone(), predef_components[8].clone(), predef_components[4].clone()]);
            }
            batch_count += 1;
            ...
            for comp in sector_components {
                ...
                let mut params = rmcp::model::CallToolRequestParams::new("neo_sys_queue_push");
                params.arguments = Some(serde_json::json!({ "topic": q_name_clone, "payload": payload.to_string() }).as_object().unwrap().clone());
                if let Ok(_) = worker_mcp_client.call_tool(params).await {
                    summary.push_str(&format!("- {} ({}x{})\n", comp.name, comp.width, comp.height));
                }
            }

            println!("{}", format!("[PRODUCER] ✅ Batch Complete:\n{}", summary).cyan());
            let _ = app_producer.emit("agent-event", AgentEvent { agent_id: "Producer".into(), status: "done".into(), details: summary, target_x: None, target_y: None });
...
pub async fn redraw_background_routes(
    ...
    for i in 0..(state_len - 2) {
        ...
        let s_x = start_node.get("x").and_then(|v| v.as_f64()).unwrap_or(0.0) + start_node.get("width").and_then(|v| v.as_f64()).unwrap_or(100.0) / 2.0;
        let s_y = start_node.get("y").and_then(|v| v.as_f64()).unwrap_or(0.0) + start_node.get("height").and_then(|v| v.as_f64()).unwrap_or(100.0) / 2.0;
        let e_x = end_node.get("x").and_then(|v| v.as_f64()).unwrap_or(0.0) + end_node.get("width").and_then(|v| v.as_f64()).unwrap_or(100.0) / 2.0;
        let e_y = end_node.get("y").and_then(|v| v.as_f64()).unwrap_or(0.0) + end_node.get("height").and_then(|v| v.as_f64()).unwrap_or(100.0) / 2.0;
        ...
        if let Ok(res) = client.post(&neonia_mcp_url).bearer_auth(&neonia_key).json(&payload).send().await {
            if let Ok(json) = res.json::<serde_json::Value>().await {
                if let Some(text) = json.pointer("/result/content/0/text").and_then(|v| v.as_str()) {
                    if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(text) {
                        if let Some(polyline) = parsed.get("svg_polyline").and_then(|u| u.as_str()) {
                            let old_polyline = start_node.get("outgoing_route").and_then(|v| v.as_str()).unwrap_or("");
...
pub fn spawn_consumer_task(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            println!("{}", "[RUST HOST] Polling queue... (0 LLM tokens spent)".dimmed());
            
            if let Some(item) = queue_pop(&queue_name, &neonia_key, &neonia_mcp_url).await {
                items_processed += 1;
                println!("{}", format!("[RUST HOST] 📦 Popped item from queue: {}", item).green().bold());
                
                let _ = app_host.emit("queue-popped", json!({ "item": &item, "agent_id": items_processed }));

                let (item_name, item_width, item_height, svg_file) = if let Ok(json) = serde_json::from_str::<serde_json::Value>(&item) {
                    let name = json.get("name").and_then(|n| n.as_str()).unwrap_or(&item).to_string();
                    ...
                    let svg_file = json.get("svg_file").and_then(|v| v.as_str()).unwrap_or("").to_string();
                ...
                tokio::spawn(async move {
                    ...
                    println!("{}", format!("{} 🚀 Spawned! Spatial reasoning...", prefix).yellow());

                    let _ = worker_app.emit("agent-event", AgentEvent { 
                        agent_id: agent_name.clone(), status: "started".into(), details: format!("Placing {}", item_name), target_x: None, target_y: None
                    });
                    
                    match agent.prompt(&format!("Place the {} on the factory layout and generate its SVG.", item_name)).with_hook(DemoHook { prefix: prefix.clone(), agent_id: agent_name.clone(), app: worker_app.clone() }).await {
                        Ok(res) => {
                            let _ = worker_app.emit("agent-event", AgentEvent { agent_id: agent_name.clone(), status: "done".into(), details: "Finished!".into(), target_x: None, target_y: None });
                            ...
                            if let Some(start) = res.find('{') {
                                if let Some(end) = res.rfind('}') {
                                    ...
                                    if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(json_str) {
                                        ...
                                        let mut final_x = parsed.get("x").and_then(|v| v.as_f64()).unwrap_or(0.0);
                                        let mut final_y = parsed.get("y").and_then(|v| v.as_f64()).unwrap_or(0.0);
                                        
                                        {
                                            let mut state = layout_clone.lock().unwrap();
                                            let (fx, fy) = find_valid_position(final_x, final_y, item_width, item_height, &state);
                                            final_x = fx; final_y = fy;

                                            new_component["x"] = serde_json::json!(final_x);
                                            new_component["y"] = serde_json::json!(final_y);
                                            new_component["id"] = serde_json::json!(format!("{}_{}", item_name, agent_id));
                                            new_component["component_id"] = serde_json::json!(format!("{}_{}", item_name, agent_id));
                                            new_component["width"] = serde_json::json!(item_width);
                                            new_component["height"] = serde_json::json!(item_height);
                                            new_component["allow_overlap"] = serde_json::json!(false);
                                            state.push(new_component);
                                        }

                                        let svg_content_opt = if !svg_file.is_empty() {
                                            if let Ok(content) = std::fs::read_to_string(format!("assets/components/{}", svg_file)) { Some(content) } else { None }
                                        } else if let Some(uri) = parsed.get("svg_uri").and_then(|u| u.as_str()) {
                                            let clean_uri = uri.trim_matches(|c| c == '"' || c == '\'' || c == '.' || c == ',' || c == '`' || c == '*' || c == '<' || c == '>' || c == '{' || c == '}' || c == '[' || c == ']');
                                            fetch_resource!(worker_mcp_client, clean_uri)
                                        } else { None };

                                        if let Some(svg_content) = svg_content_opt {
                                                let instances = vec![serde_json::json!({ "x": final_x, "y": final_y, "size": item_width.max(item_height) })];
                                                ...
                                                let previous_block = if state_len >= 2 { Some(routes_to_redraw[state_len - 2].clone()) } else { None };

                                                if state_len >= 3 {
                                                    tokio::spawn(redraw_background_routes(neonia_key_inner.clone(), neonia_mcp_url_inner.clone(), worker_app.clone(), layout_clone.clone(), routes_to_redraw, state_len));
                                                }
                                                ...
                                                if let Some(prev) = previous_block {
                                                    ...
                                                    let current_layout_json = {
                                                        let state = layout_clone.lock().unwrap();
                                                        serde_json::to_string(&state.iter().take(state.len().saturating_sub(2)).collect::<Vec<_>>()).unwrap_or_else(|_| "[]".to_string())
                                                    };

                                                    let prev_x = prev.get("x").and_then(|v| v.as_f64()).unwrap_or(0.0);
                                                    let prev_y = prev.get("y").and_then(|v| v.as_f64()).unwrap_or(0.0);
                                                    let prev_w = prev.get("width").and_then(|v| v.as_f64()).unwrap_or(100.0);
                                                    let prev_h = prev.get("height").and_then(|v| v.as_f64()).unwrap_or(100.0);
                                                    ...
                                                    tokio::spawn(async move {
                                                        let agent = router_or_client.agent("google/gemini-3-flash-preview").preamble(&router_prompt).default_max_turns(5).rmcp_tools(router_tools, router_peer).build();
                                                        let prefix = format!("[ROUTER-{}: {}]", agent_id, item_name);
                                                        println!("{}", format!("{} 🔌 Routing...", prefix).magenta());
                                                        
                                                        if let Ok(res) = agent.prompt("Connect the buildings.").with_hook(DemoHook { prefix: prefix.clone(), agent_id: r_agent_name.clone(), app: router_app.clone() }).await {
                                                            if let Some(start) = res.find('{') {
                                                                if let Some(end) = res.rfind('}') {
                                                                    let json_str = &res[start..=end];
                                                                    if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(json_str) {
                                                                        if let Some(polyline) = parsed.get("svg_polyline").and_then(|u| u.as_str()) {
                                                                            let path_d = format!("M {}", polyline.replace(" ", " L "));
                                                                            let route_id = format!("route_{}", Uuid::new_v4().simple());
                                                                            let svg_content = format!(
                                                                            ...
                                                                            let clean_uri = uri.trim_matches(|c| c == '"' || c == '\'' || c == '.' || c == ',' || c == '`' || c == '*' || c == '<' || c == '>' || c == '{' || c == '}' || c == '[' || c == ']');
                                                                            if let Some(svg_content) = fetch_resource!(r_mcp_client, clean_uri) {
                                                                                let _ = router_app.emit("svg-generated", json!({ "agent_id": r_agent_name, "svg": svg_content, "instances": [json!({"x": 0.0, "y": 0.0, "size": 1200.0})], "item_name": format!("Route to {}", item_name) }));
                                                                            }
                                                                        }
                                                                    }
                        ...
                        Err(e) => {
                            let _ = worker_app.emit("agent-event", AgentEvent { agent_id: agent_name.clone(), status: "error".into(), details: e.to_string(), target_x: None, target_y: None });
                        }
                    }
                });
